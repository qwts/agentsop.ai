...
// every url link must answer. Links on the site's own domain are generated by
// the build and covered by the tests, so they are skipped unless --live is
// passed (DNS for the domain converges outside this repository). Network is
// required; GITHUB_TOKEN (or GH_TOKEN) raises the API rate limit when present
// but is never required for public sources.
//
//   node tools/pins-check.mjs [--json] [--live]
...
export async function checkPins(routes, { includeSite = live } = {}) {
  const failures = [];
  const seen = new Set();
  const siteOrigin = `https://${routes.domain}/`;
  ...
  for (const zone of routes.zones) {
    for (const section of zone.sections ?? []) {
      for (const link of section.links ?? []) {
        ...
        if (!includeSite && url.startsWith(siteOrigin)) continue;
        seen.add(url);
